...
import { uploadPastYearAction } from "@/app/actions/pastYearAction";

export default function PastYearUploadForm() {
  ...
  const handleSubmit = async (formData: FormData) => {
    ...
    startTransition(async () => {
      try {
        const result = await uploadPastYearAction(formData);
  ...
  return (
    <form action={handleSubmit} className="space-y-5.5">
      <InputGroup
        type="text"
        label="Past Year Title"
        placeholder="Enter past year title"
        value={title}
      ...
      {success && (
        <div className="text-green-500 text-sm bg-green-50 dark:bg-green-900/20 p-3 rounded">
          Past year uploaded successfully!
        </div>
      )}
      ...
      >
        {isPending ? 'Uploading...' : 'Upload past year'}
      </button>
    </form>
  );
}